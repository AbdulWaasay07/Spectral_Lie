...
def extract_acoustic_features(waveform: np.ndarray, sr: int = config.SAMPLE_RATE) -> dict:
    ...
    # These are high-CPU. Skip them if USE_PRAAT_FEATURES is False to prevent timeout.
    if not config.USE_PRAAT_FEATURES:
        utils.logger.info("Skipping Praat features (USE_PRAAT_FEATURES=False)")
